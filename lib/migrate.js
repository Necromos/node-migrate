...
var initMongoClient = require('./mongodb-init');

module.exports = function (set, direction, migrationName, fn) {
  initMongoClient(function(err, mongoClient) {
    if (err) return fn(err);
    return migrate(mongoClient, set, direction, migrationName, fn);
  })
};

function migrate(mongoClient, set, direction, migrationName, fn) {
  var migrations = []
  var lastRunIndex
  var toIndex
  ...
  function next (migration) {
    ...
    var returnValue = migration[direction](mongoClient, function (err) {
