...
const completeMigration = (err, next, migration, migrations, direction, set, mongoClient, callback) => {
  ...
  set.save((err) => {
    ...
    next(migrations.shift(), migrations, direction, set, mongoClient, callback);
  });
}

const next = (migration, migrations, direction, set, mongoClient, callback) => {
  ...
  const returnValue = migration[direction](
    (err) => {
      ...
      completeMigration(err, next, migration, migrations, direction, set, mongoClient, callback);
    },
    mongoClient
  );
  ...
  if (isPromise) {
    returnValue
      .then(() => completeMigration(null, next, migration, migrations, direction, set, mongoClient, callback))
      .catch(callback);
  }
}
...
const migrate = (mongoClient, set, direction, migrationName, callback) => {
  ...
  return next(migrations.shift(), migrations, direction, set, mongoClient, callback);
}
